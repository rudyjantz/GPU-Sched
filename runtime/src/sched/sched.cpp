...
const long GTX_1080_TOTAL_MEM_KB  = 8116L * 1024;
    ...
    #define NUM_GPUS 4
    #define INIT_GPU_RES()          \
        assert(0 && "Finish defining GPU_RES_CC_4. It currently holds dummy values"); \
    ...
    #define NUM_GPUS 0
    #define INIT_GPU_RES() \
        assert(0 && "Must define a valid GPU resource (e.g. GPU_RES_SLOP, \
               GPU_RES_CC_2, etc.)");
#endif
...
#define BEMPS_SCHED_LOG(str) \
  do { \
    std::cout << get_time_ns() << " " << __FUNCTION__ << ": " \
              << str; \
                  std::cout.flush(); \
  }while(0)
#else
...
void usage_and_exit(char *prog_name)
{
    ...
    printf("\n");
    printf("    where which_scheduler is one of zero, round-robin,\n" \
...
static inline
void dump_gpu_res(const char *which_env)
{
    int i;
...
static inline
void set_wakeup_time_ns(struct timespec *ts_p)
{
    struct timespec now;
...
void sched_vector(void)
{
    ...
    while(1){
        ...
        pthread_cond_timedwait(&bemps_shm_p->gen->cond, &bemps_shm_p->gen->lock, &ts);
...
void sched_no_beacons(int is_round_robin)
{
    ...
    while(1){
        ...
        // TODO spurious wakeups ? shouldn't make a big difference to wake up
        // randomly from time to time before the batch is ready
        rc = pthread_cond_timedwait(&bemps_shm_p->gen->cond, &bemps_shm_p->gen->lock, &ts);
